...
class Attendance extends G_Controller {
	...
	public function show(){
		...
		$myDeptCode = $this->data["myInfo"]->userDeptCode;
		$myDeptName = $this->data["myInfo"]->userDepartment;
		$myDesignation = $this->data["myInfo"]->userDesignation;
		
		$myDept[$myDeptCode] = $myDeptName;
		...
		} else if ( $myDesignation == "Supervisor" ) {
	        
	        $this->data['departmentLists'] = $myDept;
	        $depts = array_keys($myDept);
	        $this->data['staff_array'] = $this->user_model->getStaffArray($depts);
	    ...
	    if( ($emp_id != $this->myEmpId)){
            if( ($this->data['isManager']) && (!$this->data['isAdmin'] && !$this->data['isManagement']) || ($myDeptCode == "CA" && $myDesignation == "Supervisor") ){
...
die(); */
		...
		$ramadan_dates=$this->ramadan_model->get_ramadan();
		$this->data["ramadan"] = $ramadan_dates;
	    //load view page    
        $this->data["title"] = "report";
		$this->data["sub_title"] = "Attendance Report";
		...
		if( $logInSec <= ($officeStartSec + $flexibleSec) ) {
		    ...
		    if($incidents[$logInDate]) {
		        ...
		        if( ($logInSec - $officeStartSec ) > 3600 ) {
	       		...
	       		$login_status = "<td class='txt-grn-bold topTips' title='".$this->LateReq[$logInDate]." on ".$logInDate."&#013 Login time:".$logInTime."'> ".date("h:i:s a", $officeStartSec). "<sup style=' color:#491313;'>*</sup> </td>";
...
//                         $return['msg'] = "Mail isn't sent successfully.";
	...
	public function todays_employee()
	{
		$this->isLoggedIn();
		if($this->uri->segment(4)){
		...
		$req_page = $this->uri->segment(3) ? $this->uri->segment(3) : "all";
		$array = array(
		    'dept_code'=>"all",
		    'select'=>array(
		        'e.emp_id',
		        'e.name',
		        'min(i.stime) as stime',
		        'e.office_stime',
		        'e.dept_code',
		        'e.designation'
		    )
		);
		$this->data['todays_all_users'] = $this->attendance_model->get_todays_employee(date("Y-m-d"),$array);
		$total_row = count($this->data['todays_all_users']);
		$this->data["base_url"] = base_url() . "attendance/todays_employee/$req_page/";
		$array['offset'] = $this->data["offset"] = ($page - 1) * ROWS_PER_PAGE;
		$this->pagination->initialize(paginationAttribute($this->data,$total_row,$page,$req_page));
		
		$this->data['todays_users']=$this->attendance_model->get_todays_employee(date("Y-m-d"),$array);
		$this->data['designations'] = array_column($this->user_model->designation(),'designation','id');
		$this->data['all_dept'] = array_column($this->user_model->get_all_dept(),'dept_name','dept_code');
		$str_links = $this->pagination->create_links();
		$this->data["links"] = explode('&nbsp;',$str_links );
		$this->data["status_array"] = $this->status_array;
		$this->data["title"] = $req_page;
		$this->data["sub_title"] = "Todays employee";
		
		$this->load->view('todays_employee', $this->data);

	}
	public function attendance_by_dept()
	{
		...
		$this->data["base_url"] = base_url() . "attendance/attendance_by_dept/";
		$this->data['todays_all_users'] = $this->attendance_model->get_todays_employee(date("Y-m-d"),$array);
		$total_row = count($this->data['todays_all_users']);
		$array['offset'] = $this->data["offset"] = ($page - 1) * ROWS_PER_PAGE;
		$this->pagination->initialize(paginationAttribute($this->data,$total_row,$page,$req_page));

		$this->data['todays_users'] = $this->attendance_model->get_employees_by_dept(date("Y-m-d"),$dept_code,$array);
		$this->data['designations'] = array_column($this->user_model->designation(),'designation','id');
		$this->data['all_dept'] = array_column($this->user_model->get_all_dept(),'dept_name','dept_code');

		if(count($this->data['todays_users'])<0)
			$this->data['error'] = "error-message";
		
		/* echo '<pre>';
		print_r($this->data);
		die; */
		$this->load->view('ajax_employee_by_dept', $this->data);
	}
	public function upload_training_attendance()
	{
		//$this->isLoggedIn();
	   ...
	   // }
		if(isset($_FILES['textFile'])) {
			...
			$ext = substr($filename,-3);
			$rename_file=strtotime("now");
			if (!file_exists(FCPATH.'training_attendance')) {
				mkdir(FCPATH.'training_attendance',777);
			}
			$files1 = scandir(FCPATH.'training_attendance');

			$res=in_array($filename,$files1);
			if (!in_array($filename,$files1)) {
				if(strtoupper($ext)=="TXT"){
					$config['upload_path'] = FCPATH.'training_attendance';
					...
					try {
						...
						if (!$this->upload->do_upload('textFile')) {
							...
							$this->data['message'] = "File don't uploaded";
							$this->view('error', $this->data);
						}else{
							$filepath = FCPATH."training_attendance"."/".$rename_file.'.'.$ext;
							$this->upload_attendence_data($filepath);
							$this->data['message'] = "Attendance data upload successfully";
						}
				...
				$this->data['message'] = "File Name Already Exist";   
				
			}	
	    }else{
	        $this->data['message'] = "File type is invalid. Please upload valid text file.";   
		}
		// $this->data['message'] = "<span style='color: green'>Done; $count entry(s) added to database<span>";
	...
	private function upload_attendence_data($filepath){
		$processData = $this->process_attendence_file($filepath);

		unlink($filepath);
		if($processData['success'] == true){
			$res = $this->attendance_model->insertAttendenceDataFromCsv($processData['filepath']);
            if($res){
                unlink($processData['filepath']);
                return true;
            }
            return false;
		}
	...
	private function process_attendence_file($filepath){

//		$success = false;
//		$file = fopen($filepath,"r");
//		$fileArr = [];
//		$csvFilePath = FCPATH.'training_attendance'.DIRECTORY_SEPARATOR.'attendence_'.strtotime("now").'.csv';
//        $csvFilePathTest = FCPATH . 'training_attendance' . DIRECTORY_SEPARATOR . 'attendence_' . strtotime ( 'now' ) .rand(1,1000).'.csv';
//		$fp = fopen($csvFilePath, 'w');
//        $fp2 = fopen($csvFilePathTest, 'w');
//		$dataStoreArray = [];
//		$dateStoreArray = [];
//		$empIdStoreArray = [];
//
...
//			$arr = explode("\t",$content);
//            fputcsv($fp2, $arr);
//            $vrType = isset($arr[7]) ? trim($arr[7]) : "";
//			if($vrType == 1){
//				$dateArr = explode(" ",$arr[1]);
//				$dataStoreArray[$dateArr[0]][$arr[0]][] = $arr ;
//				if(!in_array ( $dateArr[0], $dateStoreArray , TRUE)){
//                    $dateStoreArray[] = $dateArr[0];
//                }
//                if(!in_array ( $arr[0], $empIdStoreArray , TRUE)){
//                    $empIdStoreArray[] = $arr[0];
//                }
//			}
//		}
//
...
//        try {
//
//            foreach ($dateStoreArray as $key=>$ditem) {
//                foreach ($empIdStoreArray as $eitem){
//                    $info =[];
//                    $arrayFirstElement = $dataStoreArray[ $ditem ][ $eitem ][ 0 ] ?? null;
//                    if (isset( $dataStoreArray[$ditem][$eitem])){
//                        $countTotalNumberOfArray = count ($dataStoreArray[$ditem][$eitem]);
//                    }else{
//                        $countTotalNumberOfArray  = 0 ;
//                    }
//
//                    $arrayLastElement  = $countTotalNumberOfArray > 1 ? $dataStoreArray[ $ditem ][ $eitem ][ $countTotalNumberOfArray-1 ] : null;
//                    if($arrayFirstElement !=null){
//
//                        $sDateTime = explode ( ' ' , $arrayFirstElement[ 1 ] );
//                        $info[ 'emp_id' ] = sprintf ( '%04d' , trim ( $arrayFirstElement[ 0 ] ) );
//                        $info[ 'stime' ] = $sDateTime[ 1 ];
//                        if($arrayLastElement != null){
//                            $eDateTime = explode ( ' ' , $arrayLastElement[ 1 ] );
//                            $info['etime'] =  $eDateTime[1];
//                        }else{
//                            $info['etime'] =  null;
//                        }
//                        $info[ 'date' ] = $sDateTime[ 0 ];
//                        $info[ 'system_fault' ] = 'N';
//                        fputcsv($fp, $info);
//                        $this->attendance_model->insertAttendenceData($info);
//                                echo "<pre>";
...
//		];
        ...
        while ( ! feof ( $file )) {
            ...
            $arr     = explode ( "\t" , $content );
            $vrType  = isset( $arr[ 7 ] ) ? trim ( $arr[ 7 ] ) : '';
            if ( $vrType == "Only Fingerprint" ) {
                $dateArr                                           = explode ( ' ' , $arr[ 0 ] );
                $key                                               = trim ( $arr[ 1 ] );
                $fileArr[ $key ][ $dateArr[ 0 ] ][ $dateArr[ 1 ] ] = $arr;
            }
        }
        if ( isset( $fileArr[ 'Personnel ID' ] ) ) {
            unset( $fileArr[ 'Personnel ID' ] );
        }
        /*foreach ($fileArr as $arrKey => $arrVal) {
            foreach ($arrVal as $subArrKey => $subArrVal) {
                // Sort the array
                usort ( $subArrVal , function ( $element1 , $element2 ) {
                    $datetime1 = strtotime ( $element1[ 7 ] );
                    $datetime2 = strtotime ( $element2[ 7 ] );
                    return $datetime1 - $datetime2;
                } );

                $firstItem = $subArrVal[ 0 ];
                if ( count ( $subArrVal ) > 1 ) {
                    $lastItem = end ( $subArrVal );
                }

                $sDateTime             = explode ( ' ' , $firstItem[ 1 ] );
                $startInfo[ 'emp_id' ] = sprintf ( '%04d' , trim ( $firstItem[ 0 ] ) );
                $startInfo[ 'stime' ]  = $sDateTime[ 1 ];
                $startInfo[ 'etime' ]  = '';
                $startInfo[ 'date' ]  = $sDateTime[ 0 ];
                $startInfo[ 'system_fault' ] = 'N';

                if ( isset( $lastItem ) ) {
                    $eDateTime           = explode ( ' ' , $lastItem[ 0 ] );
                    $endInfo[ 'emp_id' ] = sprintf ( '%04d' , trim ( $lastItem[ 1 ] ) );
                    $endInfo[ 'etime' ]  = '';
                    $startInfo[ 'stime' ]  =$eDateTime[ 1 ];
                    $endInfo[ 'date' ]  = $eDateTime[ 0 ];
                    $endInfo[ 'system_fault' ] = 'N';
                }

                fputcsv ( $fp , $startInfo );
                if ( isset( $lastItem ) ) {
                    fputcsv ( $fp , $endInfo );
                }
            }
        ...
        foreach ($fileArr as $arrKey => $arrVal) {
            foreach ($arrVal as $subArrKey => $subArrVal) {
                ...
                $sDateTime             = explode ( ' ' , $firstItem[ 0 ] );
                $startInfo[ 'emp_id' ] = sprintf ( '%04d' , trim ( $firstItem[ 1 ] ) );
                $startInfo[ 'stime' ]  = $sDateTime[ 1 ];
                $startInfo[ 'sdate' ]  = $sDateTime[ 0 ];
                if ( isset( $lastItem ) ) {
                    $eDateTime           = explode ( ' ' , $lastItem[ 0 ] );
                    $endInfo[ 'emp_id' ] = sprintf ( '%04d' , trim ( $lastItem[ 1 ] ) );
                    $endInfo[ 'etime' ]  = $eDateTime[ 1 ];
                    $endInfo[ 'edate' ]  = $eDateTime[ 0 ];
                }
                fputcsv ( $fp , $startInfo );
                if ( isset( $lastItem ) ) {
                    fputcsv ( $fp , $endInfo );
