...
class Remark extends G_Controller {
    ...
    public function updateNotice($aData = array(), $addF=false, $needToRedirect = true){
        ...
        if($addF == true) {
            ...
            $data['notice_date'] = $this->input->post('noticeDate');
            $data['subject'] = $this->input->post('noticeSubject');
            $data['notice'] = $this->input->post('noticeBody');
            $data['isEncrypted'] = $this->input->post('isEncrypted');
            $page = $this->input->post('page_number');
            //echo base64_decode($data['notice']);
        }
        ...
        if(empty($id)){
            ...
            if($addF == false){
                $data['notice'] = base64_decode($data['notice']);
            }
            ...
            if($insert_id){
                ...
                $emailBody = "<table width='90%' align='center'>
                                <tr>
							    <td colspan='3'>
                ...
                /*$obj = new stdClass();
                $obj->name = "Mahtab Uddin Shahed";
                $obj->email = "shahed.blackmagic@gmail.com";
                $receiver1[] = $obj;

                $obj1 = new stdClass();
                $obj1->name = "Mahtab Uddin Shahed";
                $obj1->email = "shahedbhuiyan.uiu@live.com";
                $receiver1[] = $obj1;*/
