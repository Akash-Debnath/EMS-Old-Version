			...
			if(!$isBlocked) {
				...
				if (empty($errMsg)) {
					if (filter_var($login_id, FILTER_VALIDATE_EMAIL)) {
						...
						// normal Employee ID
						if (!preg_match("/^[a-zA-Z0-9]{2,10}$/", trim($login_id))) {
							//$this->user_model->addFailedLog($login_id);
...
// 	    $this->load->view('login', $data);	
	...
	public function login_dhakatel($user_name, $pass){
    ...
    public function SetSessionSettings($loggedEmpId='')
	{
	...
	public function detail($emp_id = "") 
	{
		...
		$this->data['experience'] = $user['experience'];
		//echo $user['image'];
		//Right Panel
...
// 		$this->data['emp_ids'] = $this->user_model->get_all_emp_id();
	...
	public function edit($emp_id, $pData = array())
	{
	    $this->data['designations'] = $this->user_model->designation();
	    $this->data['grades'] = $this->user_model->grade();
	    $this->data['status_array'] = $this->status_array;
        ...
        if(empty($emp_id) && count($pData) > 0){
			...
			$info->office_stime = $user['office_stime'];
			$info->office_etime = $user['office_etime'];
	...
	Public function updateEmployee(){
		...
		$pData['office_stime'] = $this->input->post('stime');
	    $pData['office_etime'] = $this->input->post('etime');
	    ...
	    $pData['present_address'] = str_replace(array("\\n", "\\r"), ' ', $this->input->post('pre_address'));
		$pData['permanent_address'] = str_replace(array("\\n", "\\r"), ' ', $this->input->post('per_address'));
        ...
        if(empty($id)){
            ...
            if($flag){
                ...
                $nData['subject'] = 'Reference: GSL/ Admin & HR/ Joining of New Employee.';
			...
			$oldData['office_stime'] = $this->input->post('stime');
            $oldData['office_etime'] = $this->input->post('etime');
            ...
            $oldData['present_address'] = str_replace(array("\\n", "\\r"), ' ', $this->input->post('pre_addressH'));
			$oldData['permanent_address'] = str_replace(array("\\n", "\\r"), ' ', $this->input->post('per_addressH'));
	...
	public function changeArchive($emp_id) {
	...
	public function facilities($emp_id){
	    $this->isLoggedIn();
	...
	public function getLoginDay($date){
	    date_default_timezone_set("Asia/Dhaka");
	...
	public function getLoginTime($date){
	    date_default_timezone_set("Asia/Dhaka");
    ...
    public function getImagePath($image){
	    ...
	    $headers = get_headers($imageURL, 0, $context);
	...
	public function add_status_log($emp_id){
	...
	public function del_status_log($status_id){
	...
	public function change_grade($grade_id){
	...
	public function add_grade($grade_value){
	...
	public function update_profile()
	{
		...
		$field_name=$this->input->post('column');
		$value=$this->input->post('value');
		$old_value=$this->input->post('old_value');
		$all=$this->user_model->get_employee_edit_list();
		// $this->user_model->employe_edit_insert($id,$column,$value);
		if ($field_name=="dob") {
			$value=date_format(date_create($value),'Y-m-d');
			// echo $value;
			// die();
		}
		if($this->validate_update_profile($value,$old_value,$field_name)){
			if (in_array($id,array_column($all,'emp_id'))){
				$this->user_model->employe_edit_update($id,$field_name,$value);
			}else{
				$this->user_model->employe_edit_insert($id,$field_name,$value);
			}
		}else{
			echo "error";
		}
	}
	public function validate_update_profile($value,$old_value,$field_name)
	{
		if ($value=="" || $value==$old_value) {
			return false;
			// return "null";
		}else{
			if ($field_name=="mobile" && strlen($value) != "11" || $field_name=="phone" && strlen($value) != "11"){
				return false;
			}else{
				return true;
			}
	...
	public function edit_history()
	{
		...
		$edited_history=$this->user_model->get_update($emp_id);
		$this->data['edited']=$edited_history;
		...
		$this->data["sub_title"] = "See Edit History";
		$this->data["notSelect"] = true;
		
		$this->load->view('edit_history', $this->data);
		// $this->load->view('edit_history');
	}
	public function delete_Edit()
	{
		...
		$value=$this->input->post('value');
		$this->user_model->user_delete_edit($id,$column,$value);

		redirect(base_url().'user/edit_history');
	}
	public function see_update_request()
	{
		$this->isLoggedIn();
		if(!$this->data['isAdmin']) {
		$this->load->view('not_found', $this->data);
		...
		$updated_request=$this->user_model->updated_request();
		$this->data['updated_request']=$updated_request;
		...
		$this->data["sub_title"] = "Profile Update Request";
		$this->data["notSelect"] = true;
		
		$this->load->view('updated_request', $this->data);
	}
	public function reject_Update()
	{
		$status="R";
		$id=$this->input->post('id');
		$this->user_model->reject_Update($id,$status);
		redirect(base_url().'user/see_update_request');
	}
	public function approve_Update()
	{
		...
		$status="A";
		$id=$this->input->post('id');
		$update_value=$this->user_model->get_update($id);
		$this->user_model->approve_Update($id,$status,$update_value);
		$this->user_model->delete_Update($id);
		redirect(base_url().'user/see_update_request');
	}
