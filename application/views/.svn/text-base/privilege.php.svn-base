	...
	<div class="col-lg-7">
		<div class="row">
			...
			<div class="col-md-6">
				...
				<div class='box'>
					<div class='box-header'>
						<h3 class='box-title'>Roster Privilege</h3>
					</div>
					<div class='box-body no-padding'>
						<table class="table">
							<thead>
												...
												<tbody>
													<tr>
														...
														<td>
															<form class="" method="post"
																action="<?php echo base_url();?>settings/delete_priviledge">
																<input type="hidden" value="<?php echo $obj->emp_id ?>" name="emp_id">
																<input type="hidden" value="R" name="type">
																<input type="hidden" value="<?php echo !empty($obj->dept_code) ? $obj->dept_code : ""; ?>" name="dept_code">
																<div class="hover-btn">
																	<button type="submit"
																		onclick="return confirm('Do you want to delete?')"
																		class="close">X</button>
											...
											<form
												...
												<table>
													<tbody>
														<tr>
															<td><select name="select_emp_id" class="selectpicker"
																id='select_roster' data-live-search="true" data-width="100%">
																	<option value="">Select Employee</option>
                                    			...
                                    			<?php foreach ($employees as $dept=>$ary){ 
                                    				...
                                    				foreach($ary as $obj){
                                    					echo "<option value='".$obj->emp_id."' deptcode='".$obj->dept_code."'>".$obj->emp_id." - ".$obj->name."</option>";
                                    				}
                                    				echo "</optgroup>";
                                    			} ?>
                                    		</select> <input type="hidden"
																name="type" value="R"> <input type="hidden"
																name="dept_code" value="" id="dept_code_roster"></td>
															<td><button type="submit" class="btn btn-primary btn-xs"
																	style='margin-left: 4px;'>
																	<span class="glyphicon glyphicon-plus"></span> Add
																</button></td>
...
//     $("#myform").submit();
    ...
    $("#select_roster").change(function(){
        var selectedDept = $("#select_roster").find(':selected').attr('deptcode');
        if(typeof selectedDept != 'undefined' && selectedDept != null && selectedDept != ""){
            $("#dept_code_roster").val(selectedDept);
            //$("#dept_code_roster").attr('value', selectedDept);
        }
    });
});
