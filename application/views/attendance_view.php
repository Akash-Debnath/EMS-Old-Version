...
<link
	...
	<link
	href="<?php echo base_url();?>assets/lib/tipsy/tipsy.css"
	type="text/css" rel="stylesheet" />
<?php
    //print_r($departmentLists);
     
?>	
<?php
$mySessionId = $this->session->GetLoginId();
//if (!empty($mySessionId) && ($mySessionId == '0146' || $mySessionId == '0181')){
if (!empty($mySessionId) && $mySessionId == '0181'){
?>
<div class='row' style='margin: -10px 0 18px 0;'>
    <div class='col-md-12 text-center'>
        <a href="<?php echo site_url('attendance_api_manual/auto_upload_fingerprint')?>" target="_blank" onclick="return confirm('Are you sure to run manual upload?')">Manually Upload Attendance Data</a>
    </div>
</div>
<?php
}
        ...
        <div class='col-lg-3 col-sm-6'>
			<div class="form-group">
				...
				<div class="col-sm-9">
        			...
        			foreach ($staff_array[$sel_dept] as $obj) {
        ...
        if(isset($records)){
            foreach ($records as $obj) {?>
                ...
                <tr>
					<td>
					<?php
						if ( checkRamadanDate($obj->recordDate,$ramadan) ) {
					?>
						<span style='color:red;' class='fa fa-moon-o' data-toggle='tooltip; data-placement='top' title='Ramadan month'></span>
					<?php		
...
<div class='row'>
	...
	<div class='col-lg-3 col-md-4 col-sm-6'>
		<label>Total absent day:</label> <?php if($controller->totalAbsent == 0) echo $controller->totalAbsent; else echo "<span class='txt-red-bold'>".$controller->totalAbsent."</span>"?>
	</div>
	<div class='col-lg-3 col-md-4 col-sm-6'>
...
<script
	src="<?php echo base_url();?>assets/lib/tipsy/jquery.tipsy.js"
	type="text/javascript"></script>
