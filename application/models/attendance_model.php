...
class Attendance_model extends G_Model{
	...
	public function getHolidays($empInfo, $sdate, $edate) {
	    ...
	    if($empInfo["roster"]=="Y"){
	        ...
	        $sql = "SELECT hr.date FROM roster_holiday AS hr WHERE hr.emp_id='".$empInfo["emp_id"]."' AND hr.date>='$sdate' AND hr.date<='$edate'";
	        $query = $this->db->query($sql);
			$data = $query->result_array();
			
			foreach ($data as $row) {
				$temp_date = $row['date'];
				$holidays[$temp_date] = 'Roster Holiday';
			}
...
// 	    $this->db->or_where('date', $ioRecordDate);
	...
	public function run_query(){
		...
		public function insertAttendenceDataFromCsv($filePath){

		$file = str_replace('\\', '/', $filePath);

        $sql   = '(@val,@val1,@val2) SET emp_id = @val, stime = @val1, etime = \'00:00:00\', date = @val2, system_fault = \'N\'';
            $query = "LOAD DATA  INFILE '" . $file . "'  INTO TABLE iorecords
    ...
    /**
     * Insert Multiple Row  in io records table from formatted array
     *
     * @param $dataArray
     * @return      bool
     */
    public function insertAttendenceData ( $dataArray )
    {
        $status = false;
        try {
            $this -> db -> insert( 'iorecords' , $dataArray );
            $status = true;
        } catch ( Exception $e ) {
	...
	public function get_todays_employee($date,$array=array())
	{
		$this->db->select($array['select']);
		$this->db->from('iorecords i');
		$this->db->join('employee e', 'i.emp_id = e.emp_id', 'right');
		$this->db->where('i.date =', $date);
		$this->db->group_by('i.emp_id');
	...
	public function get_employees_by_dept($date,$dept_code,$array=array())
	{
		$this->db->select($array['select']);
		$this->db->from('iorecords i');
		$this->db->join('employee e', 'i.emp_id = e.emp_id', 'right');
